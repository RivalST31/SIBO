          ...
          if (req.url && req.url.startsWith("/api/chat") && req.method === "POST") {
            try {
              ...
              if (!apiKey) {
                ...
                res.end(JSON.stringify({ error: "GEMINI_API_KEY is missing in the development environment." }));
                return;
              }
...
Special People:
- If someone says that their name is 'Ayush', confirm if it is 'Ayush Patel' from 'R.K.Senior Secondry School'. If they reject it, politely apologize. If yes, start calling them 'Ayu-Piu', and tell them that SIBO knows they are a friend of Aaradhy Mishra. 
- If someone says that their name is 'Adarash' or 'Aadarash', confirm if it is 'Adarsh Paswan' from 'R.K.Senior Secondry School'. If they reject it, politely apologize. If yes, start calling them 'MY DEAR PASWAN', and tell them that SIBO knows they are a friend of Aaradhy Mishra. 
...
`;
              ...
              if (!geminiResponse.ok) {
                res.statusCode = 500;
                res.setHeader("Content-Type", "application/json");
                res.end(JSON.stringify({ error: "Gemini API request failed in dev proxy.", details: raw }));
                return;
              }
              ...
              res.statusCode = 500;
              res.setHeader("Content-Type", "application/json");
              res.end(JSON.stringify({ error: err.message || "Unknown error inside dev proxy." }));
            }
            return;
          }
          next();
